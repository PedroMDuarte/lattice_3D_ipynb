...
lattice potential is formed by retro-reflecting a red-detuned Gaussian beam, resthis results in a sinusoidal intensity pattern along the beam.  Overlapped onto
...
retro-reflected, so it does not form a standing wave potential.  
...
The lattice beam that propagates along teh $x$ axis produces a potential of the form 
\begin{equation}
 V_{L}( x; y,z) = - s_{0} \exp\left[- 2 \frac{ y^{2} + z^{2} }{w_{L}^{2}} \right]
  \cos^{2}( k_{L} x ) 
\end{equation}
...
\begin{equation}
 V_{C}( x; y, z) = g_{0}  \exp\left[ -2 \frac{ y^{2} + z^{2} }{w_{C}^{2}} \right] 
\label{eq:Vcomp}
\end{equation}
...
We use the high-temperature series expansion (HTSE),  an analytical solution to
the Hubbard model that is valid at high-temperatures\footnote{ The HTSE is very
good for $T \gg t$ and works down to $T/t \sim 1.8$ }.  This solution gives us
the thermodynamic quantities (density, double occupancy, entropy per particle,
etc.)  for a homogeneous system;  we then use the local density approximation
(LDA) to explore the properties of the sample in the compensated lattice
potential.  
...
One of the important things to note is that at each point in space $\bv{r}$  we
will, in general, have three different lattice depths, associated with each of
the $x,y,$ and $z$ lattice directions.  For this reason the potential is
difficult to visualize.  To make thinks simpler, we can plot trap profiles
along the 111 direction.   Along this direction we have that the lattice depth
\footnote{We will use the letter $s$ for the lattice depth in recoils.  In our
notation $s$ carries no units, and we will explicitly indicate when a quantity
is obtained in units of $E_{R}$. } is the same in all three
directions\footnote{We will use the notation $\rdiag$ to represent the distance
along the 111 diagonal line.}:
\begin{equation} 
...
A plot of the potential envelopes, as well as the lattice potential, is shown
in Fig.~\ref{fig:lattice_general}.
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/lattice_general.png}
    \caption{Diagonal profiles for a 10$E_{R}$ lattice with $w_{L}=47\,\mu$m. }
\label{fig:lattice_general}
\end{figure}

In the absence of interactions one can plot the local band structure.  To
simplify matters for the moment we will use the deep lattice result, where the
bandwidth, $W$, is given by~\cite{Bloch2008}.
\begin{equation}
  \frac{W}{E_{R}} =    \frac{12 t}{E_{R} }  = \frac{48}{\sqrt{\pi}} s^{3/4} e^{-2\sqrt{s}} 
\end{equation}
The lowest energy of the band, measured from the potential envelope
$V_{L,\text{env}}$, is given by the zero-point energy of the 3D harmonic
oscillator centered at the bottom of each lattice site.   Again, this harmonic
oscillator result is only valid for deep lattices ($\gtrsim 10\,E_{R}$) but we
will use it here for illustrative purposes: 
\begin{equation}
  \frac{ \frac{3}{2} \hbar \omega_{0} }{ E_{R} } = 3\sqrt{ s } 
\end{equation} 

The energy of the first excited band corresponds to an excitation along one of
the lattice directions, which adds an extra $\hbar \omega_{0}$  to the bottom
of the band.  In Fig.~\ref{fig:lattice_general} we have plotted the lowest band
(showing its bandwidth) and also the first excited band.    

Also indicated in Fig.~\ref{fig:lattice_general} is the radius, $r_{1}$, at
which the energy of the lowest band is equal to the $\rdiag = 0 $ band gap. In
order to stay well within the single band approximation of the Hubbard model,
our sample needs to have a size that is much smaller than $r_{1}$. 

\subsection{Lattice locking}

On ocassion we will raise the lattice depth suddenly, in order to freeze out any
further tunneling.   This will be useful to get the maximum possible
Debye-Waller factor when peforming a Bragg scattering measurement,  or it will
...
accross from 80\,$a_{0}$ to 61\,$a_{0}$ in 24 ms gives us nearly 100\%
...
$44\,E_{R}$ we want to stay within the single band Hubbard model as was pointed
above.   Figure~\ref{fig:lattice_lock} also shows the atom number, $N_{1}$,
corresponding to a one atom per site sample filled up to $r_{1}$. For atom
numbers equal or greater than $N_{1}$ the single band approximation breaks
down.   
\begin{figure}
        \centering
        \begin{subfigure}[t]{0.48\textwidth}
                \includegraphics[width=\textwidth]{figures/lattice_tunneling.png}
                \caption{Tunneling rate.}
...
line includes 3\,$E_{R}$ of compensation, with a compnensation beam waist of
$w_{C}=40\,\mu$m.}
...
Currently, when doing Bragg scattering measurements, we lock the lattice up to
20\,$E_{R}$.  In the Bragg experiment we use 3\,$E_{R}$ of compensation to
compensate a 7\,$E_{R}$ lattice.   We lock the lattice up to 20\,$E_{R}$ to
increase the Debye-Waller factor, and we leave the compensation on during the
lattice lock.  Having the $3\,E_{R}$ of compensation gives us a maximum atom
number of $\approx$230,000 atoms to stay in the single band regime.  The
scenario for our current experiments is shown with a dotted line in
Fig.~\ref{fig:lattice_lock}.
...
cannot afford to have any green compensation light on.  This reduces the
maximum atom number required to stay in the single band regime while locking
the lattice.  

\subsection{Characteristic energy and characteristic filling}

In the previous subsection we considered the number of atoms required to fill
up a sample (at a density of one per site) up to a certain radius.    Since the
band is curved, as shown in Fig.~\ref{fig:lattice_general}, for a given radius
there is an associated energy of the band.   The exact shape of the potential
and the number of atoms define this energy, which is referred to as the
characteristic energy of the trap, $E_{t}$.  We will give the exact definition
of $E_{t}$ in the paragraphs below.  
...
we can safely use the harmonic approximation to obtain
\begin{equation}
\begin{split}
...
\begin{equation} 
   \frac{ V_{\text{band}}(\rdiag) }{E_{R} }  = 
       \left(\frac{ 4 s_{0} - 2 \sqrt{ s_{0} } }{ w_{L}^{2} } \right)\rdiag^{2} 
       \equiv  \alpha_{\text{t}}  \left( \frac{ \rdiag}{a} \right)^{2}
\end{equation}
where $a=\lambda/2$ is the lattice spacing.  The shape of the lowest band
defines $\alpha_{\text{t}}$,  and for a given $\alpha_{\text{t}}$, the
characteristic energy of the trap is given by
\begin{equation} 
  \frac{ E_{\text{t}} }{E_{R}} =  \alpha_{\text{t}} \left(  \frac{ 3N }{ 8\pi }
\right)^{2/3} 
\end{equation}
where $N$ is the total atom number.  It is seen here that the characteristic
energy is the energy of the band at a radius given by 
\begin{equation}
  \frac{ \rdiag }{ a } =  \left( \frac{ 3N }{ 8 \pi} \right)^{1/3}
  \ \ \Rightarrow   \ \ 
  N = \frac{2}{d^{3}} \left(   \frac{4}{3} \pi \rdiag^{3}  \right)
\end{equation}
This radius is simply the size occupied by the $N$ atoms at a density of two
atoms per site.   


The characteristic energy is a useful quantity because it provides a
length-scale-free way of assesing  the level of confinement in the trap.
...
$E_{\text{t}} \approx U/2 $.   Notice that such a characteristic energy can be
achieved in either of two ways: changing the confinement, $\alpha_{\text{t}}$,
or changing the atom number, $N$.  In our compensated lattice setup, the
compensation beams provide a way to change the confinement.  

As an alternative measure of the same concept, the characteristic filling is
defined as~\cite{Jordens2010}.
\begin{equation} 
...
such that one can get access to the phases of interest.  In our case these are
the Mott insulator state and the antiferromagnetically ordered Mott-insulator
which both require a density of one atom per site.  

In order to get a rough idea of what kind of trap do we need to have in order
to realize the insulating states for a given number of atoms, it is useful to
introduce an analytic expression for the onsite interaction strength $U$.   For
a deep lattice ( $\gtrsim 10\,E_{R}$ ) we have 
\begin{equation}
...
wavelength $\lambda = 20113 a_{0}$.  For a reasonable\footnote{Very large
scattering lengths are to be avoided because they give rise to fast inelastic
losses that scale as $a_{s}^{4}$.} scattering length of 650$a_{0}$ and a
$10\,E_{R}$ lattice we obtain  $U=1.82\,E_{R} = 80 t$.   From the prescription
above, half-filling would be obtained by setting $E_{\text{t}} \approx U/2 = 40
t$.  

A plot of $E_{\text{t}}/t$ as a function of lattice beam waist for a fixed atom
number, $N$=500,000 is shown in Fig.~\ref{fig:Et-waist}.   We see that
achieving half-filling with the confinement provided only by the red-detuned
lattice requires a lattice beam waist of $\approx 126\,\mu$m.  
\begin{figure}
...
atoms. The black line denotes $U/2t$,  which should be the characteristic
energy required to obtain a half-filled sample at the center of the potential
and maximize the likelihood of observing an insulting phase.}
\label{fig:Et-waist}
\end{figure}
...
As we mentioned above, we are interested in exploring the idea of evaporative
...
waist $w_{C}$,  as in Eq.~\ref{eq:Vcomp},  we have, in the harmonic
approximation 
\begin{equation}
 \frac{ \alpha_{\text{t}}^{2} }{ a^{2} } = 
 \frac{ 4s_{0} - 2 \sqrt{s_{0}} }{w_{L}^{2} }  
  - \frac{ 4 g_{0} }{ w_{C}^{2} }   
\end{equation} 
If we define  $\alpha = w_{L}/w_{C}$, then we can completely cancel the
quadratic term of the confinement by choosing 
\begin{equation}
 g_{0} = \frac{  4 s_{0} - 2 \sqrt{s_{0}} }{ 4 \alpha^{2} }  \equiv g_{0\text{q}}
\end{equation}
A plot of the diagonal profiles of the system in this case is shown in
Fig.~\ref{fig:lattice_general-comp}.  
\begin{figure}
    \centering
\includegraphics[width=0.7\textwidth]{figures/lattice_general-comp.png}
\caption{Diagonal profiles for a 10$E_{R}$ lattice with $w_{L}=47\,\mu$m,
$w_{C}=40\,\mu$m and compensation $g_{0}=6.1\,E_{R}$.}
\label{fig:lattice_general-comp}
\end{figure}

When using compensation we will find that we can use smaller beam waists and
that the potential has the capacity to accomodate a larger number of atoms.
Since the size of the sample starts becoming comparable to the lattice beam
waist, we can no longer work in the harmonic approximation.   In later sections
we will turn to the more precise implementation of the local density
approximation, and we will calculate the relevant local quantities ($U$, $t$,
etc. ) numerically, without making any deep lattice or harmonic approximations. 
